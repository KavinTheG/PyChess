    ...
    def get_legel_moves(self, piece, board, x, y):
        ...
        if type(board.get_piece(x, y)) == Pawn:
            if not y == 0 or not y == 7:
                ...
                if (
                    y == 1
                    and type(board.get_piece(x, y + 2 * direction)) == int
                    and not piece.light
                ):
                ...
                if 0 <= new_x < 8:
                    ...
                    if left_y >= 0:
                        if (
                            ...
                            or not board.get_piece(new_x, left_y).light == piece.light
                        ):
                            possible_moves.append((new_x, left_y))
                    if right_y < 8:
                        if (
                            type(board.get_piece(new_x, right_y)) == int
                            or not board.get_piece(new_x, right_y).light == piece.light
                        ):
            ...
            possible_moves = self.legal_moves_castles(
                piece, board, x, y
            ) + self.legal_moves_bishops(piece, board, x, y)
